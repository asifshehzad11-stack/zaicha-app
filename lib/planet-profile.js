...
// English/Hindi(fallback) dignity label — bilingual-field pattern (jaise
// sign/signLatin), taake frontend currentLang ke mutabiq khud chun sake.
const DIGNITY_LABEL_EN = {
  exalted: 'Exalted',
  own: 'Own Sign',
...
function buildPlanetProfiles(data) {
  ...
  for (const name of PLANET_ORDER) {
    ...
    profiles.push({
      ...
      dignityLabelEn: DIGNITY_LABEL_EN[dignity] || DIGNITY_LABEL_EN.neutral,
      isRetrograde: !!p.isRetrograde,
      isCombust: combustSet.has(name),
      navamsaSign: typeof navSign === 'number' ? RASI_NAMES_URDU[navSign] : null,
      navamsaSignLatin: typeof navSign === 'number' ? RASI_NAMES_LATIN[navSign] : null,
      isVargottama,
    });
  }
  return profiles;
