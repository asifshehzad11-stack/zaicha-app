...
/**
 * buildCombinedTransitPredictions — "سیاروی گردش" (transit) aur "سیاروی
 * نظر" (aspect) ko ek hi jagah, ek graha ke hisab se grouped kar ke deta
 * hai — pehle ye do alag alag tabs mein tha jo disjointed feel deta tha;
 * ab har graha ka card khud uska gochar + uski khaas nazar (drishti) dono
 * ek sath dikhata hai, taake ek graha ka poora asar ek nazar mein samajh
 * aaye.
 */
function buildCombinedTransitPredictions(gocharDetails) {
  const cards = [];
  for (const planet of TRANSIT_ORDER) {
    const detail = gocharDetails[planet];
    if (!detail || !detail.house) continue;
    ...
    const modifier = dignityModifier(detail);
    const house = detail.house;
    const transitEffect = (TRANSIT_PREDICTIONS[planet] && TRANSIT_PREDICTIONS[planet][house]) || '';
    ...
    const aspects = offsets.map((offset) => {
      ...
      return {
        aspectedHouse,
        text: `${URDU_ORDINALS[aspectedHouse]} گھر پر نظر (${signification}): ${effect}`,
      };
    });
    ...
    cards.push({
      ...
      planetLabel: label,
      house,
      transitText: `${label} اس وقت آپ کے ${URDU_ORDINALS[house]} گھر میں گردش کر رہا ہے۔ ${transitEffect}${modifier}`,
      aspects,
    });
  }
  return cards;
...
// Har graha ke liye islami tarz ke upay — sadaqa/khairat aur amal, jin mein
// kisi bhi devta/bhagwan ka naam ya koi mantra shamil nahi. Din (Sunday,
// Monday, waghera) sirf classical Vedic astrology ka aik hisabi convention
// hai (kis graha ki raftar/nature se munsalik hai) — ibadat ka hissa nahi,
// is liye sirf ek "waqt ka tarteeb" ke tor par rakha gaya hai. Har amal
// authentic Islamic practice hai (sadaqa, sabr, istighfar, zikr, Quran) —
// kisi puja/mantra/deity invocation ki jagah nahi li gayi, balke poori tarah
// hata kar Islamic mutabadil rakha gaya hai.
...
function buildRemedies(facts) {
  ...
  if (facts.hasKaalSarpDosha) {
    items.push({
      key: 'kaalsarp',
      title: 'کال سرپ یوگ',
      text: 'کمبل یا تل کا صدقہ کرنا، زیادہ استغفار کرنا، اور صبر سے معاملات کو وقت دینا مفید سمجھا جاتا ہے — یہ یوگ ہر معاملے کو نہیں روکتا، صرف نتیجہ تھوڑی تاخیر سے آتا ہے۔',
    });
  }
  if (facts.isInSadeSati) {
    items.push({
...
// "Aaj ka din" ke liye har weekday ka classical hakim graha aur rang — ye
// har kisi ke liye ek jaisa hota hai (kisi ki kundli par mabni nahi), bilkul
// jaisay panchang mein 'vaara' hota hai — is liye ise categorized daily
// card ke sath ek chhota, generic flavor line ke tor par shamil kiya hai.
...
// Har life-area ka sab se numaindah (representative) ghar — AstroMatrix
// jaisi apps ke categorized daily layout se mutasir, lekin fabricated نہیں:
// har number khud kundli ke houses array se aa raha hai (lord + dignity),
// aur "aaj" wala hissa Moon ki asal gochar house se — koi random/generic
// content nahi.
const CATEGORY_HOUSE_MAP = {
  career: 10,
  wealth: 2,
  health: 1,
  relationships: 7,
...
function houseLordOutlookPhrase(lordDignity) {
  if (lordDignity === 'exalted') return 'بہت مضبوط حالت میں ہے، آج نتائج معمول سے بہتر رہنے کی توقع ہے';
  if (lordDignity === 'own') return 'اپنی راشی میں مستحکم ہے، آج معاملات سنبھلے ہوئے محسوس ہوں گے';
  if (lordDignity === 'debilitated') return 'کمزور حالت میں ہے، آج اس شعبے میں اضافی صبر اور احتیاط سے کام لیں';
  return 'درمیانی حالت میں ہے، آج کا نتیجہ آپ کی اپنی کوشش پر زیادہ منحصر ہوگا';
}

/**
 * buildDailyRoutine — AstroMatrix jaisi apps ke "categorized daily" layout
 * se mutasir: ek hi "aaj ka pegham" paragraph ke bajaye, career/wealth/
 * health/relationships/family — har category ka apna chhota card, jo us
 * category ke natal ghar ke hakim (lord) ki dignity aur Moon ki aaj ki
 * gochar position par mabni hai (dono asal computed data hain, kuch bhi
 * fabricated nahi).
 *
 * houses: data.houses (12 entries, astro-engine.buildZaichaData se)
 * moonDetail: data.gochar.details.Moon ({house, dignity, isRetrograde})
 * targetDate: Date object (weekday nikalne ke liye)
 */
function buildDailyRoutine(houses, moonDetail, targetDate) {
  const moonHouse = moonDetail && moonDetail.house;
  const moonEffect = moonHouse ? (TRANSIT_PREDICTIONS.Moon[moonHouse] || '') : '';
  ...
  const categories = Object.keys(CATEGORY_HOUSE_MAP).map((key) => {
    ...
    const label = LIFE_AREA_LABELS_UR[key] || key;
    const outlook = houseLordOutlookPhrase(houseData.lordDignity);
    const text = `${label} کے گھر (${ordinal(houseNum)}) کا حاکم ${houseData.lord} ${outlook}۔${moonEffect ? ' آج چاند کے حساب سے مجموعی ماحول: ' + moonEffect : ''}`;
    return { key, label, house: houseNum, text };
  });

  const weekday = targetDate ? WEEKDAY_LORD_UR[targetDate.getDay()] : null;
...
module.exports = {
  ...
  // reuse karta hai — taake poori app mein aspect ka hisaab (transit
  // predictions ho ya yoga-detection) hamesha EK hi jagah se aaye, kabhi
  // do jagah alag-alag na ho jaye.
  ASPECT_OFFSETS,
};
