...
/**
 * apiKey() — 2026-09-25 (session 6) ROOT-CAUSE FIX. Asif ka VedAstro
 * "Unlimited" plan Account page par Active/Healthy tha, phir bhi live app par
 * VedAstro "Free tier rate limit exceeded" de raha tha — yani VedAstro humari
 * key ko PEHCHAAN hi nahi raha tha. Copy-paste mein sab se aam ghaltiyan:
 * aage/peeche space ya newline, "..." quotes, ya "Bearer " khud saath paste
 * ho jana. Ye function key ki VALUE ko kabhi log/return nahi karta, sirf
 * saaf karta hai.
 */
function rawApiKey() {
...
/**
 * Sirf BOOLEAN diagnostics (key ki value, lambai ya koi hissa kabhi bahar
 * nahi jata) — /api/health/vedastro endpoint ke liye.
 */
function apiKeyDiagnostics() {
  const raw = String(rawApiKey());
  const clean = apiKey();
  return {
    configured: clean.length > 0,
    looksLikeVedAstroKey: /^sk_/i.test(clean),
    neededCleanup: raw.length > 0 && raw !== clean,
  };
}
...
function redactKey(text) {
  ...
  return String(text).split(encodeURIComponent(k)).join('[REDACTED]').split(k).join('[REDACTED]');
}
 ...
 * bhi lagataar 2-3 requests fail hui). [BAAD MEIN SAHIH KIYA GAYA: asal
 * wajah key ka VedAstro tak pehchane-jaane ka tariqa tha — neeche "KEY
 * BHEJNE KA TARIQA" dekhein. Retry phir bhi rakha gaya hai, kabhi-kabhar ke
 * transient burst ke liye.] Is liye, har chhoti "abhi busy hai" halat par seedha user
 ...
 */
//
// KEY BHEJNE KA TARIQA (2026-09-25 fix): pehle sirf `Authorization: Bearer`
// header bheja jata tha. VedAstro ke 2 mustaqil sources ise alag batatay hain:
//   (1) Docs (vedastro.github.io/guide): "x-api-key, APIKey, ya
//       Authorization: Bearer" headers.
//   (2) VedAstro ka apna open-source server code (github.com/VedAstro/VedAstro
//       API/ThrottleManager.cs, ExtractAPIKey) key ko sirf URL ke
//       ".../APIKey/{key}" segment se parhta hai — header dekhta hi nahi; key
//       na mile to call ko FREE tier (IP-based throttle) mein daal deta hai.
// Kyunke dono mein ikhtilaf hai, key teeno documented headers AUR URL
// segment — sab mein bheji jati hai (browser se live test kiya: aakhir mein
// "/APIKey/xxx" lagane se response par koi farq nahi parta, Status "Pass"
// hi rehta hai). Error messages mein hamesha BEGHAIR-key wala url/redacted
// text jata hai, taake key kabhi log ya frontend tak na pohnche.
async function vedastroGet(url, attempt = 1) {
  ...
  if (key) {
    ...
    headers['APIKey'] = key;
    fetchUrl = `${url.replace(/\/+$/, '')}/APIKey/${encodeURIComponent(key)}`;
  }

  const res = await fetch(fetchUrl, { headers });
  if (!res.ok) {
    const text = redactKey(await res.text().catch(() => ''));
  ...
  if (json.Status !== 'Pass') {
    throw new Error(redactKey(`VedAstro ${url} ne Status="${json.Status}" diya: ${JSON.stringify(json.Payload)}`));
  }
  return json.Payload;
}
...
// ---- Speed (2026-09-25): ek hi kundli mein kai jagah WAHI VedAstro call
// dobara hoti thi (masalan natal AllPlanetData + HouseLongitude: getPlanetPosition,
// getKundliAdvanced aur getSadeSati — teeno alag alag mangwate thay). Live test
// mein ek kundli ~75 second le rahi thi. Ab har URL ka PROMISE thori der
// (10 minute) yaad rakha jata hai: ek sath chalne wali ya foran dohrai jane
// wali calls ek hi network request share karti hain. Sirf kamyab jawab cache
// hota hai (error par entry hata di jati hai, taake agli koshish taaza ho).
// Natal URLs tareekh/waqt/jagah se bani hoti hain, is liye alag logon ka data
// kabhi mix nahi hota.
...
function cachedGet(url) {
  const now = Date.now();
  const hit = RESPONSE_CACHE.get(url);
  if (hit && now - hit.at < RESPONSE_CACHE_TTL_MS) return hit.promise;
  const promise = vedastroGet(url).catch((err) => {
    RESPONSE_CACHE.delete(url);
    throw err;
  });
  RESPONSE_CACHE.set(url, { at: now, promise });
...
/**
 * Ek (1) halki VedAstro call kar ke dekhta hai ke key pehchani ja rahi hai ya
 * FREE tier ki limit lag rahi hai. Key ki value kabhi return nahi hoti.
 */
async function probeVedAstro() {
  const url = buildUrl(
    'AllPlanetData',
    { datetime: '2000-01-01T12:00:00+05:00', coordinates: FORWARD_CALENDAR_REFERENCE_COORDINATES, ayanamsa: 1 },
    ['PlanetName', 'Sun']
  );
  const started = Date.now();
  try {
    await vedastroGet(url, 3); // attempt=3 => koi retry nahi, seedha asal jawab
    return { ok: true, ms: Date.now() - started };
  } catch (err) {
    const msg = redactKey(String((err && err.message) || err)).slice(0, 300);
    return {
      ok: false,
      ms: Date.now() - started,
      freeTierLimitHit: /free tier|rate limit|too many requests|429/i.test(msg),
      message: msg,
    };
  }
}
