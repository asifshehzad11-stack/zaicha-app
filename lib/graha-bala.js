...
function pakshaBala(planet, phaseAngle) {
  // BUG FIX (2026-09-25): pehle Sun ko 0 diya jata tha ("Sun ka paksha bala
  // nahi hota") — ye ghalat tha. Paksha Bala mein Sun, Mars, Saturn teeno
  // MALEFIC gine jate hain aur (180 - phase)/3 paate hain — 2 sources:
  // varahamihira.blogspot.com/2004/07/strength-of-planets-shadbala-i.html
  // ("the malefics are Sun, Mars and Saturn") aur dirah.org/shadbala.htm.
  // (Moon ka "double paksha bala" sirf 1 source mein mila — shamil nahi.)
  if (planet === 'Sun' || planet === 'Mars' || planet === 'Saturn') return (180 - phaseAngle) / 3;
...
function buildGrahaBala(data) {
  ...
  for (const planet of CLASSICAL_PLANETS) {
    ...
    results.push({
      ...
      // English planet name — planet key khud English mein hai (jaise Sun,
      // Moon), is liye alag lookup ki zaroorat nahi (bilingual-field pattern,
      // narrative.js ke planetLabel() jaisa).
      nameLatin: planet,
      house,
      components: {
