...
function indexPlanets(planetPositionList) {
  ...
  for (const p of planetPositionList) {
    // Ascendant (id:100) khud koi "graha" nahi hai — isay natalMap mein
    // shamil na karein, warna jo bhi code Object.keys(natalMap) se sab
    // grahas par loop lagata hai (jaise yoga-engine.js ka Kemadruma check,
    // ya navamsa.js) usay "Ascendant" naam ka ek jhoota extra graha mil
    // jata hai (D1 houses-loop mein ye masla nahi tha kyunke wahan
    // explicitly id===100 skip hota hai, lekin indexPlanets() ke naye
    // upbhoktaon ke liye ye hamesha yaad rakhna mushkil hai — is liye
    // root par hi fix kar diya).
    if (p.id === 100 || p.name === 'Ascendant') continue;
    map[p.name] = {
...
function buildZaichaData({ personName, kundliAdvanced, natalPlanetPosition, gocharPlanetPosition, sadeSati, kaalSarp, targetDate, ayanamsa }) {
  ...
  for (let h = 1; h <= 12; h++) {
    ...
    houses.push({
      ...
      // English planet name bhi (bilingual-field pattern, jaise sign/
      // signLatin) — multi-language foundation ke liye.
  ...
  return {
    person: {
      ...
      systemLatin: AYANAMSA_LABEL_EN[Number(ayanamsa)] || 'Lahiri',
    },
    natal: {
      ascendantRasiId,
      ascendantDegree: typeof ascendant.degree === 'number' ? ascendant.degree : 0, // Navamsa (D9) jaisay divisional charts ke liye zaroori — sirf sign nahi, sign ke andar exact degree bhi chahiye
      moonRasiId,
      planets: natalMap,
    },
    houses,
